...
def main():
    ...
    resp = httpx.get("https://pokeapi.co/api/v2/pokemon", headers=headers)
    ...
    for pokemon in resp.json()["results"][:3]:
        resp = httpx.get(pokemon["url"], headers=headers)

        # You don't HAVE to raise exceptions for failed requests. Can test
        # for "is_success" which passes when resp.status_code < 400
        if resp.is_success:
